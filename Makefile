...
# Set the locations of the gcc and g++ binaries. This is useful for custom builds
...
# The binary name
TARGET_BINARY ?= cpp-template

# The build directory, where the target binary will be stored
BUILD_DIR ?= ./build

# The source directory, where the source files will be stored
SRC_DIRS ?= ./src
...
# CXX flags
# CXXFLAGS ?= $(INC_FLAGS) $(MongoDB_INC_DIRS) -MMD -MP
CXXFLAGS ?= $(INC_FLAGS) -std=c++2a -MMD -MP

# Linker flags
LDFLAGS ?= -lstdc++ -lresolv -lcrypto

# Set the compiler flags
COMPILERFLAGS ?= -Wall -Wextra -Wstrict-prototypes -Wfloat-equal -Wundef -Wshadow -Wcast-align -Wstrict-overflow -Wno-stringop-overread -Wno-array-parameter -Wdouble-promotion -fexceptions -pie -fPIE -Wl,dynamicbase -Wl,nxcompat
